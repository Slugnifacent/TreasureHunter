...
namespace TreasureHunter
{
    public class Camera
    {
        ...
        public void Update() {
            ...
            if (zoom != zoomTarget)
            {
                float catchSpeed = (Avatar.Instance().speed / 50.0f);
                zoom += Movement.Approach(zoom, zoomTarget, catchSpeed);
        ...
        public Vector2 NormalizeCoordinates(Vector2 Position) {
           Vector2 direction = new Vector2(Left, TOP) - Position;
           float X = direction.X / TreasureHunter.graphics.PreferredBackBufferWidth;
           float Y = direction.Y / TreasureHunter.graphics.PreferredBackBufferHeight;
